...
# Usually the operator with the most pairs and the one with the most journeys
# differ - the pairs are a long tail of tiny school services while the journeys
# sit on a few large ones - but they can be the same operator, and then a
# "while" contrast reads as though two operators were named.
top_pairs <- names(ops)[1]
top_runs  <- names(by_runs)[1]
tail_txt <- if (identical(top_pairs, top_runs)) {
  sprintf(paste0("**%s** contributes both the most pairs (%d) and ",
                 "the most journeys (%s of %s)"),
          top_pairs, as.integer(ops[1]), fmt(by_runs[1]),
          fmt(sum(m_all$tnds)))
} else {
  sprintf(paste0("**%s** contributes the most pairs (%d) while ",
                 "**%s** contributes the most journeys (%s of %s)"),
          top_pairs, as.integer(ops[1]), top_runs, fmt(by_runs[1]),
          fmt(sum(m_all$tnds)))
}
